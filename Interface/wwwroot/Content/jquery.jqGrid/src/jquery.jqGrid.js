...
$.extend($.jgrid,{
	...
	clearBeforeUnload : function( jqGridId ) {
		...
		$(User).unbind("mouseup.jqGrid" + $t.p.id ); 
	...
	gridUnload : function ( jqGridId ) {
		...
		var newtable = User.createElement('table');
	...
	return this.each( function() {
		...
		var ts= this, grid={
			...
			dragEnd: function( events ) {
				...
				this.curGbox = null;
				User.onselectstart=function(){return true;};
			},
		...
		if(User.UserMode !== undefined ) { // IE only
			if(User.UserMode <= 5) {
		...
		firstr += "</tr>";
		tbody = User.createElement("tbody");
		//$(this).append(firstr);
		...
		thead = null;
		grid.hDiv = User.createElement("div");
		...
		grid.bDiv = User.createElement("div");
		...
		grid.cDiv = User.createElement("div");
		...
		if( ts.p.toolbar[0] ) {
			...
			grid.uDiv = User.createElement("div");
			if(ts.p.toolbar[1] === "top") {$(grid.uDiv).insertBefore(grid.hDiv);}
			else if (ts.p.toolbar[1]==="bottom" ) {$(grid.uDiv).insertAfter(grid.hDiv);}
			if(ts.p.toolbar[1]==="both") {
				grid.ubDiv = User.createElement("div");
		...
		$(User).bind( "mouseup.jqGrid" + ts.p.id, function () {
...
//Helper functions
	...
	createModal : function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {
		...
		var self = this,
			...
			mw  = User.createElement('div');
		css = $.extend({}, css || {});
		mw.className= "ui-jqdialog " + classes.modal;
		mw.id = aIDs.themodal;
		var mh = User.createElement('div');
...
// Form Functions
	createEl : function(eltype,options,vl,autowidth, ajaxso) {
		...
		{
			case "textarea" :
				elem = User.createElement("textarea");
				if(autowidth) {
			...
			break;
			case "select" :
				elem = User.createElement("select");
	...
	return this.each( function() {
		...
		*/
		...
		this.createTableRowForRule = function(rule, group ) {
			...
			//User.createElement("tr"),

			// first column used for padding
			//tdFirstHolderForRule = User.createElement("td"),
		...
		return this.each(function() {
			...
			if ($("#"+alertIDs.themodal)[0] === undefined) {
				if(!o.alerttop && !o.alertleft) {
					...
					} else if (User.UserElement !== undefined && User.UserElement.clientWidth !== undefined && User.UserElement.clientWidth !== 0) {
						o.alertleft = User.UserElement.clientWidth;
						o.alerttop = User.UserElement.clientHeight;
					} else {
						o.alertleft=1024;
						o.alerttop=768;
					}
			...
			var sortable_opts = {
				...
				"placeholder": {
					element: function(item) {
						var el = $(User.createElement(item[0].nodeName))
...
$.jqDnR={
	...
	stop:function(){
		//E.css('opacity',M.o);
		$(User).unbind('mousemove',J.drag).unbind('mouseup',J.stop);
	}
};
...
i=function(e,h,k,aR){
	return e.each(function(){
		...
		h.bind('mousedown',{e:e,k:k},function(v){
			...
			$(User).mousemove($.jqDnR.drag).mouseup($.jqDnR.stop);
			return false;
		});
	});
},
...
$.fn.tinyDraggable = function(options){
	...
	return this.each(function(){
        ...
        mousedown: function(e){
			...
			$(User).on('mousemove.drag', function(e){ el.offset({top: e.pageY-dy, left: e.pageX-dx}); });
			},
			mouseup: function(e){ $(User).off('mousemove.drag'); }
		});
	});
};
...
L=function(t){$(User)[t]("keypress",m)[t]("keydown",m)[t]("mousedown",m);},
...
window.jqGridUtils = {
	...
	xmlToJSON : function ( root, options ) {
		...
		if ( root.nodeType !== 11 ) {            // User_FRAGMENT_NODE
			var tmp = {};
